...
fn vector_of_i64x16(size: usize) -> Vec<[i64; 16]> {
	let mut rng = rand::thread_rng();
	let mut vec = Vec::with_capacity(size);
	for _ in 0..size {
		let mut arr = [0; 16];
...
fn bench_std_sort(c: &mut Criterion) {
	...
	group.bench_with_input(BenchmarkId::new("i64", 16), &0, |b, _| {
		let mut v = vector_of_i64x16(size);
		b.iter(|| {
			black_box({
				for i in 0..size {
					v[i].sort();
...
fn bench_simd_bitonic_sort(c: &mut Criterion) {
		...
		b.iter(|| {
			black_box({
				for i in 0..size {
					let arr = v[i];
					let (a, b) = arr.split_at(8);
					let (a, b) = (i64x8::from_slice(a), i64x8::from_slice(b));
					let (a, b) = bitonic_sort_i64x16(a, b);
					let a = a.as_array();
					let b = b.as_array();
					v[i][..8].copy_from_slice(a);
					v[i][8..].copy_from_slice(b);
	...
	group.bench_with_input(BenchmarkId::new("i32 (dual)", 16), &0, |b, _| {
		let mut v = vector_of_i32x16(size);
		b.iter(|| {
			black_box({
				for i in 0..size {
					...
					let (a, b) = arr.split_at(8);
					let (a, b) = (i32x8::from_slice(a), i32x8::from_slice(b));
					let (a, b) = dual_bitonic_sort_i32x16(a, b);
					let a = a.as_array();
					let b = b.as_array();
					v[i][..8].copy_from_slice(a);
					v[i][8..].copy_from_slice(b);
